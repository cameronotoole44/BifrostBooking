...
async function generateSeats() {
    ...
    for (const flight of flights) {
        const totalSeats = flight.seats;
        const seats = [];

        for (let row = 1; row <= totalSeats / 6; row++) {
            for (let col = 0; col < 6; col++) {
                const seatNumber = `${row}${String.fromCharCode(65 + col)}`; // GENERATES SEAT NUMBERS LIK 1A, 1B, ETC //
                seats.push({
                    flightId: flight.id,
                    seatNumber,
                    status: 'available',
...
generateSeats()
    .then(() => console.log('Seats generated successfully'))
    .catch((error) => console.error('Error generating seats:', error));
